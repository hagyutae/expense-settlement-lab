...
2. `src/core/domain/rules/rXXX_영문설명.py` 에 규칙을 만든다.
3. `tests/rules/test_rXXX.py` 를 만든다. 픽스처 `tests/rules/fixtures/rXXX.csv` 는 이미 있으므로 새로 만들지 않는다.
4. `src/application/container.py` 의 `build_service` 와 `tests/container.py` 의 `all_rules` 에 등록한다.
5. `uv run python ${CLAUDE_SKILL_DIR}/scripts/check_rule.py <규칙 코드>` 로 연동 지점 다섯 곳(규칙 구현·테스트·픽스처·앱 조립·테스트 조립)이 모두 반영됐는지 확인한다. **경로를 줄이지 말고 위에 적힌 그대로 실행한다.** 빠진 곳이 있으면 채우고 다시 돌린다.
6. `uv run pytest` 로 전체가 통과하는지 확인한다.
